...
const GuestForm = () => {
  ...
  return (
    <form className="flex gap-4 w-full font-mono">
      <Input placeholder="Masukkan Nama Tamu" onValueChange={setGuestName} />
      <Button
        variant="flat"
        onPress={() => {
      ...
      <section className="container-xs flex flex-col gap-6">
        <div className="flex flex-col gap-4 w-full items-center">
          <Image
            ...
            radius="none"
            src="/images/separator-003.svg"
          />
          <div className="font-sans text-2xl text-foreground-500">
            Undangan Pernikahan
          </div>
          <div className="flex flex-col items-center">
            <div className="font-bold text-5xl max-md:text-4xl">
              {process.env.NEXT_PUBLIC_LHS_FULL_NAME}
            </div>
            <div className="text-3xl">Bersama dengan</div>
            <div className="font-bold text-5xl max-md:text-4xl">
              {process.env.NEXT_PUBLIC_RHS_FULL_NAME}
            </div>
          </div>
        </div>
        <div className="flex flex-row gap-4 justify-center font-mono text-md uppercase">
          <div>{moment(marriageDate).format("DD")}</div>
          <Divider className="w-[1px] h-6 bg-black" orientation="vertical" />
